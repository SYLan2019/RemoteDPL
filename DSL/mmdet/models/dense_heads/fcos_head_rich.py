...
class FCOSHead_richness(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        #myj 8.15不直接在fpn后面跟整个richness头，而是直接加在
        ...
        #self.conv_richness = nn.Conv2d(self.feat_channels, 1, 3, padding=1)
    ...
    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'centernesses', 'richness_scores'))
    def loss(self,
             cls_scores,
             bbox_preds,
             centernesses,
        ...
        labels, bbox_targets, richness_targets = self.get_targets(all_level_points, gt_bboxes,
                                            gt_labels)
        # else:
        #     all_level_points = self.get_points(featmap_sizes, bbox_preds[0].dtype,
        ...
        if gt_bboxes_ignore is not None:
                ...
                ig_labels, _, _ = self.get_targets(all_level_points, gt_bboxes_ignore, gt_labels_ignore)
        ...
        #myj 12.12修改中心度权重，使得其在对损失计算的影响减小
        rich_weight = 0.1
        if len(pos_inds) > 0:
            ...
            loss_richness = self.loss_centerness(
                pos_richness, pos_richness_targets, weight=flatten_weights * rich_weight, avg_factor=num_pos)
    ...
    def get_bboxes(self,
        ...
        result_list = self._get_bboxes(cls_score_list, bbox_pred_list,
                                       centerness_pred_list, richness_pred_list, mlvl_points,
    ...
    def _get_bboxes(self,
        ...
        #和centerness一样
        mlvl_richness = []
        for cls_score, bbox_pred, centerness, richness, points in zip(
                cls_scores, bbox_preds, centernesses, richness_scores, mlvl_points):
        ...
        if with_nms:
            ...
            for (mlvl_bboxes, mlvl_scores,
                 mlvl_centerness, mlvl_richness) in zip(batch_mlvl_bboxes, batch_mlvl_scores,
    ...
    def get_targets(self, points, gt_bboxes_list, gt_labels_list):
        ...
        labels_list, bbox_targets_list, richness_targets_list = multi_apply(
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, points, regress_ranges,
        ...
        if num_gts == 0:
            return gt_labels.new_full((num_points,), self.num_classes), \
                   gt_bboxes.new_zeros((num_points, 4)), gt_labels.new_full((num_points,), 0.0)
