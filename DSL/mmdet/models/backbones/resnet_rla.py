...
class RLA_Bottleneck(BaseModule):
    ...
    def forward(self, x, h):
        ...
        if self.averagePooling is not None:
            ...
            # else:
            #     h = nn.functional.pad(h, (0, 1, 0, 1), mode='constant', value=0)
            #     #print('h的形状为：', h.shape)
            #     h = self.averagePooling(h)
            #     #print('下采样之后h的形状为：', h.shape)
        out += identity
        out = self.relu(out)
    ...
    def _forward_impl(self, x):
        ...
        for layers, bns, conv_out, recurrent_conv in zip(self.stages, self.stage_bns, self.conv_outs, self.recurrent_convs):
            for layer, bn in zip(layers, bns):
                ...
                # RLA module updates
                y_out = conv_out(y)
                if (y_out.size(-1) == h.size(-1)+1) and (y_out.size(-2) == h.size(-2)+1):
                    h = nn.functional.pad(h, (0, 1, 0, 1), mode='constant', value=0)
                elif y_out.size(-1) == h.size(-1)+1:
                    h = nn.functional.pad(h, (0, 1, 0, 0), mode='constant', value=0)
                elif y_out.size(-2) == h.size(-2)+1:
                    h = nn.functional.pad(h, (0, 0, 0, 1), mode='constant', value=0)
                #print('h的形状为：', h.shape)
                #print('y_out的形状为：', y_out.shape)
                h = h + y_out
                h = bn(h)
                h = self.tanh(h)
                h = recurrent_conv(h)
                #print('最后的h的形状为：', h.shape, '\n')
