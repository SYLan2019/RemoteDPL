...
class SemiVOCDataset(CustomDataset):
    ...
    def load_annotations(self, ann_file):
        ...
        with open(ann_file,'r') as f:
            ...
            for line in lines:
                ...
                #myj 2023.6.7 对semivoc的data_infos进行修改
                data_infos.append(
                    dict(id=filename.replace('.png', ''), filename=filename, width=width, height=height))
                # data_infos.append(
    ...
    def get_cat_ids(self, idx):
        ...
        #myj 2023.6.5 程序报错，增加几个列表，就不报错了
        gt_bboxes = []
        gt_labels = []
        gt_bboxes_ignore = []
        gt_masks_ann = []
    ...
    def _filter_imgs(self, min_size=32):
        ...
        for i, img_info in enumerate(self.data_infos):
            ...
            with open(os.path.join(self.ann_path, name),'r') as f:
                ...
                valid_inds.append(i)
        return valid_inds
    #这这里需要进行修改，因为我们需要使用提升值，将不确定为gtbox的表示提升值作为一个判断
            ...
            for i in range(int(data['targetNum'])):
                ...
                if 'scores' in data.keys() and self.thres is not None:
                    if isinstance(self.thres, str):
                        if not os.path.exists(self.thres):
                            if data['scores'][i] < float(self.default_thres[1]) and data['scores'][i]>= float(self.default_thres[0]):
                                #表示伪标签下面具有boost值，这个值用来进行判断
                                #11。27进行修改
                                if 'boost' in data.keys():
                                    #表示处于被丢弃的部分，但是他所对应融合部分的提升值大于0.1，表示他有概率成为真实框
                                    if data['boost'][i] >= self.boost_thres:
                                        gt_bboxes.append(bbox)
                                        gt_labels.append(int(self.labelmapper['cat2id'][data['tags'][i]]))
                                        gt_masks_ann.append(None)
                                    else:
