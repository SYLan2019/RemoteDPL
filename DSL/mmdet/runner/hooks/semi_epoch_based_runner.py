...
class SemiEpochBasedRunner(EpochBasedRunner):
    ...
    def run_iter(self, data_batch, train_mode, **kwargs):
        if self.batch_processor is not None:
            ...
            #myj 进行修改，增加down_fuse参数
            outputs = self.model.train_step(data_batch, self.optimizer,
                                            **kwargs)
            #print('outputs的结果是:\n', outputs)
            #output:{'loss': tensor(10.6083, device='cuda:1', grad_fn=<AddBackward0>), 'log_vars': OrderedDict([('loss_cls', 2.1512484550476074), ('loss_bbox', 1.9425095319747925),
            # ('loss_centerness', 1.276244878768921), ('loss_sisoft', 0.0006915417034178972), ('loss', 5.370694637298584)]), 'num_samples': 3}
        else:
            if self.ema_flag:
    ...
    def train(self, data_loader, **kwargs):
        ...
        for i, data_batch in enumerate(self.data_loader):
            ...
            if self.down_fuse:
                down_batch = copy.deepcopy(data_batch)

                #针对标记数据img_metas部分
                down_batch['img_metas'].data[0][0]['img_shape'] = (int(data_batch['img_metas'].data[0][0]['img_shape'][0]/2), int(data_batch['img_metas'].data[0][0]['img_shape'][1]/2), data_batch['img_metas'].data[0][0]['img_shape'][2])
                down_batch['img_metas'].data[0][0]['pad_shape'] = (int(data_batch['img_metas'].data[0][0]['pad_shape'][0]/2), int(data_batch['img_metas'].data[0][0]['pad_shape'][1]/2), data_batch['img_metas'].data[0][0]['pad_shape'][2])
                down_batch['img_metas'].data[0][0]['scale_factor'] = data_batch['img_metas'].data[0][0]['scale_factor']/2
                #未标记数据img_metas部分
                down_batch['img_metas'].data[0][1]['img_shape'] = (
                    int(data_batch['img_metas'].data[0][1]['img_shape'][0] / 2),
                    int(data_batch['img_metas'].data[0][1]['img_shape'][1] / 2),
                    data_batch['img_metas'].data[0][1]['img_shape'][2]
                )
                down_batch['img_metas'].data[0][1]['pad_shape'] = (
                    int(data_batch['img_metas'].data[0][1]['pad_shape'][0] / 2),
                    int(data_batch['img_metas'].data[0][1]['pad_shape'][1] / 2),
                    data_batch['img_metas'].data[0][1]['pad_shape'][2]
                )
                down_batch['img_metas'].data[0][1]['scale_factor'] = data_batch['img_metas'].data[0][1]['scale_factor'] / 2
                #针对img部分进行下采样操作,得到下采样图片
                db, c, h, w = down_batch['img'].data[0].shape
                down_batch['img'].data[0] = torch.nn.functional.interpolate(down_batch['img'].data[0][:,:,:,:].clone(), (int(h/2),int(w/2)), mode='bilinear')
                #对gt_boxes进行缩减一半
                down_batch['gt_bboxes'].data[0][0] = (data_batch['gt_bboxes'].data[0][0].clone() / 2)
                down_batch['gt_bboxes'].data[0][1] = (data_batch['gt_bboxes'].data[0][1].clone() / 2)
                #gt_labels不用变动
                #gt_bboxes_ignore也要变化，虽然不参与损失的计算
                down_batch['gt_bboxes_ignore'].data[0][0] = (data_batch['gt_bboxes_ignore'].data[0][0].clone() / 2)
                down_batch['gt_bboxes_ignore'].data[0][1] = (data_batch['gt_bboxes_ignore'].data[0][1].clone() / 2)

                kwargs.update({'down_batch':down_batch})
            if self.scale_invariant:
                ...
                # tmp.shape:torch.Size([1, 3, 608, 608])
                #表示标记图片和未标记图片的pad过后的值
                ...
                tmp[:,:,:int(h/2),:int(w/2)] = cdata#左上角变成下采样的图片了
                data_batch['img'].data[0] = torch.cat((data_batch['img'].data[0],tmp),0)#在0维度进行拼接
