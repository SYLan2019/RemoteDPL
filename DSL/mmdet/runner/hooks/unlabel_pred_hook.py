...
#这里需要进行修改，因为results的里面的元素每个shape为（种类框的个数，6）,6代表的是（xmin, ymin, xmax, ymax, score, richness）
def parse_det_results(results, score_thr, reverse_mapper=None, richness_toggle=False):
    new_results = []
    for i, res_perclass in enumerate(results):
        class_id = i
        for per_class_results in res_perclass:
            #myj 进行修改,由于是计算提升值，因此将原来的richness改为boost
            if richness_toggle:
                #xmin, ymin, xmax, ymax, score, richness = per_class_results
                xmin, ymin, xmax, ymax, score, boost = per_class_results
            else:
                xmin, ymin, xmax, ymax, score = per_class_results
            #这里进行判断，如果该类的框的得分低于阈值，则将其舍弃，这里的值为0.1
            if score < score_thr:
                continue
            ...
            if richness_toggle:
                #dict_instance['richness'] = round(float(richness), 6)
                dict_instance['boost'] = round(float(boost), 6)
            new_results.append(dict_instance)
    #每个字典都是一个实体，实体含有得分，种类，边框，richness等属性
    return new_results

def gen_save_json_dict(info_dict, save_root, reverse_mapper=None, save_polygon=False, richness_toggle=False):
    ...
    if task_type == 'Det':
        if "infer_score_thre" in set(info_dict.keys()):
            ...
            dict_tmp["infer_results"] = parse_det_results(info_dict["result"], score_thr, reverse_mapper, richness_toggle=richness_toggle)
...
#myj 2023.6.6 对txt数据进行读取
def get_image_list_from_voclist(list_file, root, anno_root):
    image_path_list = []
    img_names = mmcv.list_from_file(list_file)
    for name in img_names:
        json_name = name + '.json'
        with open(os.path.join(anno_root, json_name),'r') as f:
            data = json.load(f)
            img_path = os.path.join(root, name)
            img = Image.open(img_path)
            width, height = img.size
            #目的在于寻找未标记图片中，预测到的有物体的图像
            if min(width, height) >=32 and data['targetNum']>0:
...
def save_results2file(result, image_path, image_height, image_width,
                      ...
                      task_type, vis=False, save_polygon=False, anno_root_path=None, iou=0.1, fuse=False, first_ignore=False, richness_toggle=False):
    ...
    if save_file_format == "json":
        ...
        dict_save = gen_save_json_dict(info_dict, save_root_path,
                                       ...
                                       richness_toggle=richness_toggle)
        ...
        for i in range(int(new_info['targetNum'])):
             ...
             #myj 增加richness
             if richness_toggle:
                #new_info['richness'].append(dict_save['infer_results'][i]['richness'])
                new_info['boost'].append(dict_save['infer_results'][i]['boost'])
        ...
        if fuse:
                ...
                cids = np.array(new_info['cid'], dtype=np.float32)
                if richness_toggle:
                    #richnesses = np.array(new_info['richness'], dtype=np.float32)
                    boost = np.array(new_info['boost'], dtype=np.float32)
        ...
        for i in range(0,len(id2cat)-1):
              ...
              if len(tmp_scores) == 0:
                  continue
              if richness_toggle:
                  #tmp_richnesses = richnesses[cids==i]
                  tmp_boost = boost[cids==i]
              tmp_bboxes = bboxes[cids==i,:]
              #cls_dets = np.concatenate((tmp_bboxes,tmp_scores[:,None]),axis=1)
              #cls_dets, _ = nms_op(cls_dets, iou_thr=iou)
              #myj 需要保留索引
              cls_dets, keep = nms(tmp_bboxes, tmp_scores, iou_threshold=iou, score_threshold=0.1)
              if richness_toggle:
                   #final_richnesses.extend(tmp_richnesses[keep].tolist())
                   final_boost.extend(tmp_boost[keep].tolist())
        ...
        if richness_toggle:
            #final_info["richness"]=final_richnesses
            final_info['boost']=final_boost
    ...
    with torch.no_grad():
        if task_type in {'Det', 'Sem'}:
            if flip:
                ...
                #[tensor(第一类别框的个数，6), tensor(), ...],等会从这里开始写
                #这里需要进行修改，增加unlabel_pred部分,需要修改down_fuse，这里是用来修改伪标签生产阶段 12.25修改down_fuse
                result = model(return_loss=False, rescale=True, **data, down_fuse = False)[0]
        elif task_type == 'Cls':
...
def multi_gpu_test(model, task_type, image_list,
                   ...
                   rank, world_size, anno_root_path=None, vis=False, save_polygon=False, ema_model=None, iou=0.1, fuse=False, first_ignore=False, richness_toggle=False):
    if ema_model is not None:
         model = ema_model
    ...
    for idx in range(rank, len(image_list), world_size):
        ...
        save_results2file(result, image_path, image_height, image_width,
                          save_file_format, checkpoint_name, infer_score_thre,
                          id2cat, cat2id, image_root_path, save_root_path,
                          task_type, vis, save_polygon, anno_root_path=anno_root_path, iou=iou, fuse=fuse, first_ignore=first_ignore, richness_toggle = richness_toggle)
...
class UnlabelPredHook(Hook):
    ...
    def __init__(self, kwargs, config, task_type, interval_mode='epoch', interval=1):
        #myj 增加一个属性
        self.richness_toggle = kwargs['richness']
        ...
        #myj 2023.6.6 进行修改，改成读取txt数据格式的文件，image_list是一开始预测到的未标记图像中有物体的图像目录，把没有预测到物体的图像给过滤掉了。
        self.image_list = get_image_list_from_voclist(self.image_list_file, self.image_root_path, self.anno_root_path)
    ...
    def after_train_epoch_iter(self, runner):
        ...
        if self.num_gpus > 1:
                multi_gpu_test(runner.model, self.task_type, self.image_list,
                           ...
                           runner.rank, runner.world_size, anno_root_path=self.anno_root_path, save_polygon=self.save_polygon, ema_model = ema_model, iou=self.eval_config['iou'][0], fuse=self.fuse, first_ignore = self.first_ignore, richness_toggle = self.richness_toggle)
